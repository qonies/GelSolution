...
class DeviceParams:
    ...
    # ---- 气缸类型 → 开孔位置（以满行程标定，孔位固定于缸体）----
    # 50% 缸 = 满行程时活塞走完 50% 行程后气孔才被盖住（密封段 = 50% 满行程）。
    # 水桶效应（✅用户更正）：切齿后孔位不变，有效密封段 = min(系数×满行程,
    # 实际行程)——行程仍覆盖开孔段时密封容积不随切齿缩减；行程短于开孔段
    # 时全程密封（无自由泄压段）。
